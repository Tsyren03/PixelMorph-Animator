...
def crop_and_resize(image, target_size=(1024, 1024)):
    """Crops the center of the image without distortion and resizes it."""
    h, w = image.shape[:2]
    size = min(h, w)
...
def prepare_image(image_path, target_size=(1024, 1024), to_grayscale=False):
    """Loads an image, crops/resizes it, and optionally converts to grayscale."""
...
def rearrange_pixels(source_color_img, target_gray_img):
    ...
    to the corresponding brightness locations in the target image (Static Image).
    """
...
def animate_premium(source_color_img, target_color_img, output_filename='premium_morph.mp4', num_frames=180, fps=60,
                    block_size=32):
    """Generates a high-resolution, smooth morphing animation (Video)."""
    ...
    start_y, start_x = source_sort_indices // w, source_sort_indices % w
    end_y, end_x = target_sort_indices // w, target_sort_indices % w
